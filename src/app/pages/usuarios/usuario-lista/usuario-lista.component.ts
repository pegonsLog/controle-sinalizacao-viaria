...
@Component({
    ...
    <div class="page-container">
      <div class="page-header">
        <div class="header-content">
          ...
          <div>
            <h1>Usuários</h1>
            <p class="header-subtitle">Gerenciamento de acesso ao sistema</p>
          </div>
        </div>
      ...
      <div class="card">
        <div class="card-header">
          <div class="search-box">
            ...
            <input type="text" placeholder="Buscar usuários..." [(ngModel)]="termoBusca" />
          </div>
          <div class="results-count">{{ usuariosFiltrados.length }} registro(s)</div>
        </div>
            ...
            <tbody>
              @for (item of usuariosFiltrados; track item.id) {
                <tr>
                  <td>
                    <div class="cell-user">
                      <div class="user-avatar" [class.admin]="item.perfil === 'Administrador'">
                        <app-icon name="user" [size]="18"></app-icon>
                      </div>
                      <span class="user-matricula">{{ item.matriculaUsuario }}</span>
                    </div>
                  </td>
                  <td>
                    <span class="role-badge" [class.role-admin]="item.perfil === 'Administrador'" [class.role-user]="item.perfil !== 'Administrador'">
                      <app-icon [name]="item.perfil === 'Administrador' ? 'shield-check' : 'user'" [size]="14"></app-icon>
                      {{ item.perfil }}
                    </span>
                  </td>
                  <td class="actions">
                    <a [routerLink]="['/usuarios/editar', item.id]" class="btn-action btn-edit" title="Editar">
                ...
                <tr>
                  <td colspan="3" class="empty-state">
                    <div class="empty-icon"><app-icon name="user" [size]="48"></app-icon></div>
                    <p class="empty-title">Nenhum usuário encontrado</p>
                    <p class="empty-text">Comece cadastrando um novo usuário</p>
                  </td>
                </tr>
              }
            </tbody>
  ...
  styles: [`
    ...
    .search-box:focus-within { border-color: #8b5cf6; box-shadow: 0 0 0 3px rgba(139, 92, 246, 0.1); }
    ...
    .cell-user { display: flex; align-items: center; gap: 0.875rem; }
    .user-avatar { width: 40px; height: 40px; background: linear-gradient(135deg, #e5e7eb 0%, #d1d5db 100%); border-radius: 10px; display: flex; align-items: center; justify-content: center; color: #6b7280; }
    .user-avatar.admin { background: linear-gradient(135deg, #ede9fe 0%, #ddd6fe 100%); color: #7c3aed; }
    .user-matricula { font-weight: 600; color: #1f2937; }
    .role-badge { display: inline-flex; align-items: center; gap: 0.375rem; padding: 0.375rem 0.75rem; border-radius: 20px; font-size: 0.75rem; font-weight: 600; }
    .role-admin { background: linear-gradient(135deg, #ede9fe 0%, #ddd6fe 100%); color: #6d28d9; }
...
export class UsuarioListaComponent implements OnInit {
  ...
  get usuariosFiltrados(): Usuario[] {
    ...
    return this.usuarios.filter(u =>
      u.matriculaUsuario.toLowerCase().includes(termo) ||
